    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑 Banner' : '新增 Banner'" width="560px">
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="跳转链接">
          <el-input v-model="form.link" placeholder="如 /pages/seal-tab/index" />
        </el-form-item>
...
function openDialog(row) {
  ...
  else Object.assign(form, { id: null, title: '', image: '', link: '', sort: 0, status: 1 })
...
async function handleSave() {
  saving.value = true
  try {
    if (form.id) await request.put(`/content/banners/${form.id}`, form)
    else await request.post('/content/banners', form)
